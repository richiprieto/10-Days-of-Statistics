...
fn main() {
    ...
    let prob2: f32 = prob2.trim().parse().expect("Error");
    ...
    println!("{:.2}", (1.0 - resp1) * 100.0);

    // Pregunta 2
    let x1 = (prob2 - mean_sd[0]) / (mean_sd[1] * 2.0_f32.sqrt());
    let resp2 = 0.5 * (1.0 + erf(x1));
    println!("{:.2}", (1.0 - resp2) * 100.0);

    // Pregunta 3
    println!("{:.2}", resp2 * 100.0);
